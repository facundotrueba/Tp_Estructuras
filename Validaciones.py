...
class ValidarConexiones:
    ...
    def cargar(nombre_archivo):
        try:
            datos = LectorCSV.leer_csv(nombre_archivo)
            LectorCSV.validar(datos, nombre_archivo)
        ...
        for fila in datos:
            if fila:
                try:
                    conexion = ValidarConexiones.validar(fila)
    ...
    def cargar(nombre_archivo):
        ...
        for fila in datos:
            if fila:  # chequea que no esté vacía
                try:
                    nombre = ValidarNodos.validar(fila[0])
                    Nodo.Nodo(nombre)
                except (TypeError, ValueError) as e:
...
class ValidarSolicitud:
    ...
    def cargar(nombre_archivo):
        ...
        for fila in datos:
            if fila:
                try:
                    id_carga, peso, origen, destino = ValidarSolicitud.validar(fila)
